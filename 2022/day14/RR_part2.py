...
def simulate_sand(x,y):
    ...
    while y<=max_y: # ! As the sand can only move to the line after max_y and keep still
...
ans=0
while True:
    x,y=simulate_sand(500,0)
    blocks.add((x,y)) # ! The sand beyond max_y cannot be add into the blocks
    ans+=1
    if (x,y)==(500,0):
        break
